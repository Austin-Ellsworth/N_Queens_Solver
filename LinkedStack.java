...
public class LinkedStack<E> implements Cloneable
{
   ...
   **/ 
   ...
   /**
   * Get the item at a specified value from top of stack.
   * @param value
   * @precondition
   * 	0 <= n and n < size().
   * @postcondition
   * 	The return value is the item that is value from the top
   * 	(with the top at value = 0, the next at value = 1, and so on.)
   * 	The stack is not changed.
   * @return
   * 	The object that is value nodes from the top of the stack.
   **/
   public Object itemAt(int value) {
	   int i = 0;
	   Object answer = top.getData();
	   for(Node<E> cursor = top; i < value; i++) {
